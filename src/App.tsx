...
const fakeUser = { name: 'César', photo: 'https://i.pravatar.cc/150?img=12', email: 'cesar@pg.com' };
...
export default function App() {
  ...
  const membrosDisponiveis = MEMBROS.filter(m => m !== fakeUser.name && !jaOrou.includes(m));
  ...
  const comentar = async (id: string) => {
    ...
    await updateDoc(ref, {
      comments: arrayUnion({ user: fakeUser.name, photo: profilePhoto, text: commentText }),
    });
  ...
  const confirmarPresenca = () => {
    setConfirmados([...confirmados, { nome:fakeUser.name, lanche:lancheSelecionado, hora:new Date().toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'}) }]);
        ...
        {screen === 'home' && (
          <div className="fade" style={s.page}>
            ...
            <div style={s.welcomeBox}>
              <div style={{fontSize:11,color:'#F07830',fontFamily:'Barlow Condensed',fontWeight:700,letterSpacing:2,marginBottom:3}}>BOA VINDA, {fakeUser.name.toUpperCase()}! 👋</div>
        ...
        {screen === 'perfil' && (
          <div className="fade" style={s.page}>
            ...
            <div style={{...s.card,padding:28,textAlign:'center' as const,marginBottom:14}}>
              ...
              <div style={{fontFamily:'Bebas Neue',fontSize:28,color:'#1A1A1A',letterSpacing:2}}>{fakeUser.name.toUpperCase()}</div>
              <div style={{fontFamily:'Barlow',fontSize:13,color:'#999',marginBottom:22}}>{fakeUser.email}</div>
              <div style={{display:'flex',justifyContent:'center',gap:28,marginBottom:22}}>
                {[{n:posts.filter(p=>p.user===fakeUser.name).length,label:'Posts'},{n:SONGS.length,label:'Músicas'},{n:jaOrou.length,label:'Orações'}].map(item=>(
                  <div key={item.label}>
