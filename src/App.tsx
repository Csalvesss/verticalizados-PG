...
const SONGS = [
  {
    ...
    sections: [
      ...
      },
      {
        ...
        type: 'chorus',
        lines: [
          'Eu irei para a cruz, onde pude perceber',
      ...
      },
      {
        ...
        type: 'verse',
        lines: [
          'Quão perdido estou, quão vazio o coração',
      ...
      },
      {
        ...
        lines: [
          'Eu irei para a cruz, onde pude perceber',
          'Este amor que por mim foi pago',
          'Sua mão me curou, sua morte me salvou',
          'Eu irei, eu irei para a cruz',
        ],
      },
  ...
  },
  {
    ...
    sections: [
      ...
      },
      {
        ...
        type: 'verse',
        lines: [
          'E ao entrar pelas portas do céu',
          'O verei',
...
const Ico = {
  ...
  guitar: (c = '#888') => (
    <svg
      ...
      strokeLinejoin="round"
    >
      <path d="M11 8L3.5 15.5A2.5 2.5 0 0 0 7 19l7.5-7.5" />
      <path d="M14 6l1.5-1.5a2 2 0 0 1 2.83 0l.17.17a2 2 0 0 1 0 2.83L17 9" />
...
interface Post {
  ...
  time: string;
  createdAt: any;
  comments: { user: string; photo: string; text: string }[];
  repostOf?: { user: string; text: string };
}
...
export default function App() {
  ...
  const membrosDisponiveis = MEMBROS.filter(
    (m) => m !== fakeUser.name && !jaOrou.includes(m)
  );
  ...
  // Eventos
  const evento = {
    tema: 'Emanuel – Deus Conosco',
    data: 'Sexta-feira, 07 de março',
    hora: '19h30',
    local: 'MJA Esplanada',
  };
  ...
  const [lancheSelecionado, setLancheSelecionado] = useState<string | null>(
    null
  );
  const euConfirmei = confirmados.find((c) => c.nome === fakeUser.name);

  // Perfil
  const [profilePhoto, setProfilePhoto] = useState(fakeUser.photo);
  ...
  const curtir = async (post: Post) => {
    ...
    const jaGostou = post.likes?.includes(fakeUser.name);
    await updateDoc(ref, {
      likes: jaGostou ? arrayRemove(fakeUser.name) : arrayUnion(fakeUser.name),
    });
  };
        ...
        {screen === 'home' && (
          <div className="fade" style={s.page}>
            ...
            <div style={s.welcomeBox}>
              ...
              >
                BOA VINDA, {fakeUser.name.toUpperCase()}! 👋
              </div>
              <div
                style={{
                  fontSize: 13,
              ...
              {[
                {
                  icon: Ico.music,
                  label: 'Músicas',
                  sub: `${SONGS.length} esta semana`,
                  sc: 'musicas',
                },
                {
                  icon: Ico.guitar,
                  label: 'Cifras',
                  sub: 'Para violonistas',
                  sc: 'cifras',
                },
                {
                  icon: Ico.pray,
                ...
                },
                {
                  icon: Ico.event,
                  label: 'Eventos',
                  sub: `${confirmados.length} confirmados`,
                  sc: 'eventos',
                },
              ].map((item) => (
                <div
              ...
              return (
                <div key={song.id} style={s.card}>
                  ...
                  >
                    ...
                    <div style={{ flex: 1, padding: '14px 12px' }}>
                      ...
                      <div style={s.cardHint}>
                        {open ? 'Toque para fechar' : 'Toque para ver a letra'}
                      </div>
                    </div>
                    {Ico.chevron(open)}
                    <div style={{ width: 14 }} />
                  ...
                  {open && (
                    <div
                      style={{
                        borderTop: '1px dashed rgba(240,120,48,0.25)',
                          ...
                          {sec.lines.map((line, j) => (
                            <div
                              ...
                              style={{
                                ...
                                fontSize: 13.5,
                                lineHeight: 1.85,
                                fontWeight: sec.type === 'chorus' ? 700 : 500,
                                color:
                                  sec.type === 'chorus' ? '#1A1A1A' : '#333',
                              }}
                            >
                              {line || '\u00A0'}
                            </div>
            ...
            {CIFRAS.map((c) => {
              ...
              return (
                <div key={c.id} style={s.card}>
                  <div
                    style={s.cardTop}
                    onClick={() => setOpenCifra(open ? null : c.id)}
                  >
                    <div style={s.cardNum}>
                      <span
                        style={{
                    ...
                    <div style={{ flex: 1, padding: '14px 12px' }}>
                      ...
                      <div style={s.cardHint}>
                        {open ? 'Toque para fechar' : 'Toque para ver a cifra'}
                      </div>
                    </div>
                    {Ico.chevron(open)}
                    <div style={{ width: 14 }} />
        ...
        {screen === 'oracao' && (
          <div className="fade" style={s.page}>
            ...
            <div style={{ ...s.card, padding: 22, marginBottom: 14 }}>
                  ...
                  <>
                    <div
                      style={{
                        ...
                        color: '#F07830',
                        letterSpacing: 3,
                        animation: sorteando ? 'none' : 'pop 0.4s ease',
                      }}
                    >
                      {sorteado}
                    </div>
                    ...
                    >
                      Interceda por {sorteado} esta semana 🙏
                    </div>
                  </>
                ) : (
                  <div
              ...
              {sorteado && !sorteando && (
                ...
                >
                  ...
                  >
                    O sorteio foi realizado. Ore por{' '}
                    <strong style={{ color: '#F07830' }}>{sorteado}</strong>{' '}
                    durante a semana!
                  </div>
                </div>
              )}
              {membrosDisponiveis.length === 0 && (
                ...
                >
                  ...
                  >
                    🎉 Você já orou por todos os membros!
                  </div>
                </div>
              )}
            </div>
            <div style={{ ...s.card, padding: 16 }}>
              ...
              >
                {MEMBROS.filter((m) => m !== fakeUser.name).map((m) => (
                  <div
                    key={m}
                    style={{
                      fontFamily: 'Barlow',
        ...
        {screen === 'feed' && (
          <div className="fade" style={s.page}>
            ...
            <div style={{ ...s.card, padding: 14, marginBottom: 12 }}>
              ...
              >
                <button onClick={postar} style={s.btnOrange}>
                  Publicar
                </button>
              </div>
            </div>
            ...
            {posts.map((post) => {
              ...
              return (
                <div
                  key={post.id}
                  style={{ ...s.card, padding: 14, marginBottom: 12 }}
                >
                  {post.repostOf && (
                    <div
                      style={{
                  ...
                  >
                    <img
                      src={post.photo}
                      style={{ ...s.avatarFeed, flexShrink: 0 }}
                    />
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      >
                        ...
                        <button
                          ...
                          onClick={() =>
                            setRepostingOn(
                              repostingOn === post.id ? null : post.id
                            )
                          }
                          style={{ ...s.actionBtn, color: '#888' }}
                        >
                          {Ico.repost()}{' '}
                          <span style={{ fontSize: 12 }}>Repostar</span>
                        </button>
                        {post.user === fakeUser.name && (
                          <button
                            className="post-action"
                  ...
                  {repostingOn === post.id && (
                    ...
                    >
                      ...
                      >
                        REPOSTAR COM COMENTÁRIO (opcional)
                      </div>
                      <textarea
                        value={repostText}
          ...
          <div className="fade" style={s.page}>
            ...
            {!euConfirmei ? (
              <div style={{ ...s.card, padding: 20, marginBottom: 14 }}>
                ...
                >
                  Você vai comparecer? Se quiser, selecione o que pode levar de
                  lanche. <span style={{ color: '#bbb' }}>(opcional)</span>
                </div>
                <div
                  style={{
                    fontFamily: 'Barlow Condensed',
                ...
                >
                  {LANCHES.map((l) => (
                    <button
                      key={l}
                      className="lanche-tag"
                      onClick={() =>
                        setLancheSelecionado(lancheSelecionado === l ? null : l)
                      }
                      style={{
                        fontFamily: 'Barlow',
                        fontSize: 13,
                        ...
                        cursor: 'pointer',
                        border: `1.5px solid ${
                          lancheSelecionado === l ? '#F07830' : '#e0e0e0'
                        }`,
                        background:
                          lancheSelecionado === l
                            ? 'rgba(240,120,48,0.1)'
                            : '#fff',
                        color: lancheSelecionado === l ? '#F07830' : '#555',
                        fontWeight: lancheSelecionado === l ? 700 : 400,
              ...
              {confirmados.map((c, i) => (
                <div
                  ...
                  style={{
                    ...
                    padding: '10px 0',
                    borderBottom:
                      i < confirmados.length - 1 ? '1px solid #f0f0f0' : 'none',
                  }}
                >
                  <div
                    style={{
          ...
          <div className="fade" style={s.page}>
            ...
            >
              ...
              >
                {[
                  {
                    n: posts.filter((p) => p.user === fakeUser.name).length,
                    label: 'Posts',
                  },
                  { n: SONGS.length, label: 'Músicas' },
                  { n: jaOrou.length, label: 'Orações' },
                ].map((item) => (
                  <div key={item.label}>
                    <div
                      style={{
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              style={{
                ...s.navBtn,
                background: active ? 'rgba(240,120,48,0.12)' : 'transparent',
              }}
            >
