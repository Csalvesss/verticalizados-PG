...
export function BottomNav({ screen, goTo }: Props) {
  return (
      ...
      {NAV_ITEMS.map(item => {
        ...
        return (
          <button
            ...
            style={{ ...s.navBtn, background: active ? 'rgba(240,120,48,0.12)' : 'transparent' }}
          >
            {item.icon(active ? '#F07830' : '#555')}
            <span style={{ fontFamily: 'Barlow Condensed', fontSize: 9, fontWeight: 700, letterSpacing: 0.5, color: active ? '#F07830' : '#555' }}>
              {item.label}
            </span>
          </button>
        );
