...
export function MusicasScreen({ songs, goTo }: Props) {
            ...
            {open && (
              <div style={{ borderTop: '1px dashed rgba(240,120,48,0.25)', padding: '16px 16px 4px' }}>
                {song.sections && song.sections.map((sec, i) => (
                  <div key={i} style={{
                    marginBottom: 16,
                    ...(sec.type === 'chorus' ? { background: '#F07830', borderRadius: 12, padding: '12px 14px' } : {}),
                    ...(sec.type === 'bridge' ? { borderLeft: '4px solid #F07830', paddingLeft: 12 } : {}),
                  }}>
                    <div style={{ fontFamily: 'Barlow Condensed', fontSize: 9, fontWeight: 700, letterSpacing: 3, textTransform: 'uppercase', marginBottom: 5, color: sec.type === 'chorus' ? 'rgba(0,0,0,0.4)' : '#D4621A' }}>
                      {sec.label}
                    </div>
                    {sec.lines.map((line, j) => (
                      <div key={j} style={{ fontFamily: 'Barlow', fontSize: 13.5, lineHeight: 1.85, fontWeight: sec.type === 'chorus' ? 700 : 500, color: sec.type === 'chorus' ? '#1A1A1A' : '#333' }}>
                        {line || '\u00A0'}
                      </div>
                    ))}
                  </div>
                ))}
                {song.letra && !song.sections?.length && (
                  <div style={{ fontFamily: 'Barlow', fontSize: 13.5, lineHeight: 2, color: '#333', whiteSpace: 'pre-wrap' }}>{song.letra}</div>
                )}
              </div>
            )}
